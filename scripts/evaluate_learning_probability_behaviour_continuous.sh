...
EXPERIMENT_TYPE=learning_based_probability_behaviour
...
for ORDER_ARRIVAL_INTERVAL in ${ORDER_ARRIVAL_INTERVAL_LIST[*]}; do
	...
	for S_DECENTRALISED_LEARNING_PROBABILITY in ${NUMBER_OF_ROBOTS_LIST[*]}; do

		echo ${S_DECENTRALISED_LEARNING_PROBABILITY}

		if [[ ${S_DECENTRALISED_LEARNING_PROBABILITY} -eq 25 ]]; then
		    NUMBERS_OF_HOURS=06                                                                                                    
		    NUMBERS_OF_MINS=00
		...
		for EXPLORATION_PROBABILITY in ${EXPLORATION_PROBABILITY_LIST[*]}; do
			...
			JOB_PARAM="S${S_DECENTRALISED_LEARNING_PROBABILITY}_WORKING_THRESH_${WORKING_THRESH}_MINBH_${MIN_BH}_ORDER_ARRIVAL_AVERAGE${ORDER_ARRIVAL_INTERVAL}_EXPLORATION_PROBABILITY_${EXPLORATION_PROBABILITY}"
