...
DATA_RECORDING_INTERVAL=3600 # s
...
sed -e "s|TIME_STEPS|${TIME_STEPS}|" \
	-e "s|SPEED|${SPEED}|" \
	-e "s|MAX_BH|${MAX_BH}|" \
	-e "s|LOW_THRESH|${LOW_THRESH}|" \
	-e "s|SAFETY_THRESH|${SAFETY_THRESH}|" \
...
HIGH_THRESH_LIST=(60.0 80.0 100.0)

for HIGH_THRESH in ${HIGH_THRESH_LIST[*]}; do
	...
	for S_NAIVE in ${NUMBER_OF_ROBOTS_LIST[*]}; do
		...
		for MIN_BH in ${MIN_BH_LIST[*]}; do
			...
			JOB_PARAM="S${S_NAIVE}_HIGH_THRESH_${HIGH_THRESH}_MINBH_${MIN_BH}"
